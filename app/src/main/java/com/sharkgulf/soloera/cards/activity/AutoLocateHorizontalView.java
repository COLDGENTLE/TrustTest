...
public class AutoLocateHorizontalView extends RecyclerView {
    ...
    private String TAG = AutoLocateHorizontalView.class.getCanonicalName();
    ...
    private void init() {
        getViewTreeObserver().addOnGlobalLayoutListener(new ViewTreeObserver.OnGlobalLayoutListener() {
            ...
            public void onGlobalLayout() {
                if (isInit) {
                    ...
                    TrustLogUtils.d(TAG,"selectPos:"+selectPos);
        ...
        mBackgroudPaint.setColor(Color.parseColor("#00000000"));
        mDashLinePaint = new Paint(Paint.ANTI_ALIAS_FLAG);
        mDashLinePaint.setStyle(Paint.Style.STROKE);
        mDashLinePaint.setColor(Color.TRANSPARENT);
    ...
    /**
     * 设置初始化时选中的位置,该方法必须在{@link AutoLocateHorizontalView#setAdapter(RecyclerView.Adapter) }之前调用
     *
    ...
    public void setStartPos(int initPos){
        this.initPos = initPos;
        selectPos = initPos;
        oldSelectedPos = initPos;
        deltaX = 0;
    ...
    /**
     * 设置每次显示多少个item,该方法必须在{@link AutoLocateHorizontalView#setAdapter(RecyclerView.Adapter) }之前调用
     *
    ...
    public void onScrollStateChanged(int state) {
        ...
        if (state == SCROLL_STATE_IDLE) {
            ...
            TrustLogUtils.d(TAG,"overLastPosOffset : "+overLastPosOffset);
            TrustLogUtils.d(TAG,"Math.abs(overLastPosOffset) <= itemWidth / 2 : "+(Math.abs(overLastPosOffset) <= itemWidth / 2));
            TrustLogUtils.d(TAG,"overLastPosOffset > 0 : "+(overLastPosOffset > 0));
            TrustLogUtils.d(TAG,"else : "+(-(itemWidth + overLastPosOffset)));
            calculateSelectedPos();
    ...
    class WrapperAdapter extends RecyclerView.Adapter {
        ...
        public ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
            if (viewType == HEADER_TYPE) {
                View view = LayoutInflater.from(parent.getContext()).inflate(R.layout.item_alh_add_view,parent,false);
                ...
                View view = LayoutInflater.from(parent.getContext()).inflate(R.layout.item_alh_add_view,parent,false);
                headerFooterWidth = parent.getMeasuredWidth() / 2 - (parent.getMeasuredWidth() / itemCount) / 2 +30;
                RecyclerView.LayoutParams params = new LayoutParams(headerFooterWidth, ViewGroup.LayoutParams.MATCH_PARENT);
                view.setLayoutParams(params);
                return new HeaderFooterViewHolder(view);
            }
        ...
        private void moveItemToCenter(View itemView)
        {
            ...
            smoothScrollBy((locations[0] + rbWidth / 2 - screenWidth / 2),0,new LinearInterpolator());
        }

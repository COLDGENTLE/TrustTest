...
import kotlinx.android.synthetic.main.fragment_controll_ctrl_tmpl_two.*

/**
 *  Created by user on 2019/8/21
 */
class FragmentControllCtrlTmplTwo  : TrustMVPFragment<IControllCarView, ControllCarPresenter>(), IControllCarView  {
    ...
    override fun getLayoutId(): Int {
        return R.layout.fragment_controll_ctrl_tmpl_two
    }
    private var mControllCarCallBack: FragmentControllCar.ControllCarCallBack? = null
    fun setControllCarCallBack(callback:FragmentControllCar.ControllCarCallBack):FragmentControllCtrlTmplTwo{
    ...
    override fun initView(view: View?, savedInstanceState: Bundle?) {
        baseSetOnClick(controll_two_lock_btn)
        baseSetOnClick(controll_two_start_btn)
        baseSetOnClick(controll_two_unlock_btn)
        baseSetOnClick(controll_two_find_btn)
        baseSetOnClick(controll_two_bucket_btn)
        baseSetOnClick(controll_two_no_voice_lock_btn)
    }
    ...
    override fun baseResultOnClick(v: View) {
        when (v.id) {
            R.id.controll_two_lock_btn -> {
                mControllCarCallBack?.carLock(ACTION_OPEN)
            }
            R.id.controll_two_start_btn -> {
                mControllCarCallBack?.carStart(true)
            }
            R.id.controll_two_unlock_btn -> {
                mControllCarCallBack?.carLock(ACTION_CLOSE)
            }
            R.id.controll_two_find_btn -> {
                mControllCarCallBack?.findCar(true)
            }
            R.id.controll_two_bucket_btn -> {
                mControllCarCallBack?.carBucket(true)
            }
            R.id.controll_two_no_voice_lock_btn -> {
                mControllCarCallBack?.carLock(ACTION_NO_MUSIC_CLOSE)
            }
            else -> {
            }
        }
